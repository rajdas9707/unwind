...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[overthinking] GET / - userId=${userId} date=${date} category=${category} page=${page} limit=${limit}`
    );
    ...
    console.log(
      `[overthinking] GET / - Success: Found ${entries.length} entries, total: ${total}`
    );
    res.json(response);
  } catch (error) {
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!entry) {
      console.log(
        `[overthinking] GET /:id - Not found: id=${id} userId=${userId}`
      );
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const { title, thought, solution, date, dumped = false } = req.body;
    ...
    if (!thought || !date) {
      console.log(
        `[overthinking] POST / - Bad request: Missing required fields`
      );
    ...
    } = require("../llm/services/overthinkingService");

    const aiProcessedData = await processOverthinkingEntry(
      title,
      thought,
      solution
    );
    console.log(
      `[overthinking] POST / - AI processing complete:`,
      aiProcessedData
    );
    ...
    const entryData = {
      userId,
      thought: aiProcessedData.thought, // ✅ Use AI-generated summary, not raw text
    ...
    console.log(
      `[overthinking] POST / - Creating entry with AI-processed data:`,
      entryData
    );
    ...
    console.log(
      `[overthinking] POST / - Success: Created entry ${savedEntry._id} for user ${userId}`
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!entry) {
      console.log(
        `[overthinking] PUT /:id - Not found: id=${id} userId=${userId}`
      );
      return res.status(404).json({ error: "Overthinking entry not found" });
    }
    ...
    if (thought !== undefined) {
      ...
      console.log(
        `[overthinking] PUT /:id - Re-processing with AI due to thought update...`
      );
      const {
        processOverthinkingEntry,
      } = require("../llm/services/overthinkingService");
      ...
      console.log(
        `[overthinking] PUT /:id - AI re-processing complete:`,
        aiProcessedData
      );
    ...
    console.log(
      `[overthinking] PUT /:id - Success: Updated entry ${id} for user ${userId}`
    );
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!userId) {
      console.log(
        `[overthinking] DELETE /:id - Unauthorized: No userId in token`
      );
      return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    if (!entry) {
      console.log(
        `[overthinking] DELETE /:id - Not found: id=${id} userId=${userId}`
      );
      return res.status(404).json({ error: "Overthinking entry not found" });
    }
    ...
    console.log(
      `[overthinking] DELETE /:id - Success: Deleted entry ${id} for user ${userId}`
    );
...
router.patch("/:id/dump", async (req, res) => {
  ...
  try {
    ...
    if (!userId) {
      console.log(
        `[overthinking] PATCH /:id/dump - Unauthorized: No userId in token`
      );
      return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    if (!entry) {
      console.log(
        `[overthinking] PATCH /:id/dump - Not found: id=${id} userId=${userId}`
      );
      return res.status(404).json({ error: "Overthinking entry not found" });
    }
    ...
    console.log(
      `[overthinking] PATCH /:id/dump - Success: Dumped entry ${id} for user ${userId}`
    );
    res.json(updatedEntry);
  } catch (error) {
...
router.get("/stats", async (req, res) => {
  ...
  try {
    ...
    if (!userId) {
      console.log(
        `[overthinking] GET /stats - Unauthorized: No userId in token`
      );
      return res.status(401).json({ error: "Unauthorized" });
    }
