...
export const listOverthinkingEntries = async ({ date, page = 1, limit = 50 } = {}) => {
  ...
  const response = await axios.get(`${API_BASE_URL}/api/overthinking${queryString}`, {
    headers,
    timeout: 10000,
  });
...
export const createOverthinkingEntry = async ({ thought, solution, date, dumped = false }) => {
  ...
  const response = await axios.post(`${API_BASE_URL}/api/overthinking`, {
    thought,
    solution,
    date,
    dumped,
...
export const updateOverthinkingEntry = async ({ id, thought, solution, dumped }) => {
  ...
  const response = await axios.put(`${API_BASE_URL}/api/overthinking/${id}`, {
    thought,
    solution,
    dumped,
  }, {
...
export const dumpOverthinkingEntry = async ({ id }) => {
  ...
  const response = await axios.patch(`${API_BASE_URL}/api/overthinking/${id}/dump`, {}, {
    headers,
    timeout: 10000,
  });
...
export const deleteOverthinkingEntry = async ({ id }) => {
  ...
  const response = await axios.delete(`${API_BASE_URL}/api/overthinking/${id}`, {
    headers,
    timeout: 10000,
  });
