...
export const listJournalEntries = async ({
  ...
  const response = await axios.get(
    `${API_BASE_URL}/api/journal${queryString}`,
    {
      headers,
      timeout: 10000,
    }
...
export const createJournalEntry = async ({ content, title }) => {
  ...
  // console.log("createJournalEntry called with", { content, title });
  ...
  console.log("result from listjournal/api/journal", response);
...
export const updateJournalEntry = async ({ id, content, title }) => {
  const token = await getFreshToken();
